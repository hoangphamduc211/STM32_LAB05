...
int __alloc(struct pcb_t *caller, int vmaid, int rgid, int size, int *alloc_addr)
{
   ...
   */
  inc_limit_ret = inc_vma_limit(caller, vmaid, inc_sz, &inc_limit_ret);
  ...
  if(inc_limit_ret == 0){
    ...
    *alloc_addr = rgnode.rg_start;
    ...
    printf("Increase vma limit failed \n");
    return -1;
  }
...
int validate_overlap_vm_area(struct pcb_t *caller, int vmaid, int vmastart, int vmaend)
{
  ...
  struct vm_area_struct *vma_cur = caller->mm;
  while(vma_cur){
    if(vma_cur->vm_id != vmaid){
      if(!(vmastart >= vma_cur->vm_end || vmaend <= vma_cur->vm_start)){
        printf("This memory range is overlapped by vmaid: %d \n", vma_cur->vm_id);
        return -1;
      }      
    }
